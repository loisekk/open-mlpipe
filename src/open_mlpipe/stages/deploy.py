...
class DeployStage(Stage):
    ...
    def should_skip(self, ctx: PipelineContext) -> bool:
        return not (ctx.config and ctx.config.deployment.enabled)
    ...
    def execute(self, ctx: PipelineContext) -> PipelineContext:
        ...
        self._generate_api(ctx, config, output_dir)

        # Generate Dockerfile
        self._generate_dockerfile(ctx, config, output_dir)

        return ctx

    def _generate_api(self, ctx, config, output_dir):
...
app = FastAPI(title="{config.project} API")
...
'''
    ...
    def _generate_dockerfile(self, ctx, config, output_dir):
        """Generate Dockerfile."""
        docker_code = f'''FROM {config.deployment.docker_base_image}
...
CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "{config.deployment.port}"]
'''
